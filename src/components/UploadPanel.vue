<template>
    <Row class="upload-panel">
        ...
        <Row class="upload-space" v-show="is_show"
            ...
            <div class="el-dragger__text">点击上传</div>
        </Row>
    ...
    export default {
        ...
        methods: {
            deleteVideo(){
                this.is_show = true
                this.resourse_url = ''
                this.$emit('deleteVideo') // 线上培训 编辑视频
            },
            handleSave() {
                //获取剪切后的图片对象并且上传到服务器
            ...
            //通过插件显示图片
            handleChangeMedia(e) {
                this.isVideo = this.$config.checkVideo(e.target.files[0].type)
                ...
                if(this.isType !== 'video') this.fullscreenLoading = this.$LoadingY({message: '', show: true})
                if(this.isType == 'video') this.fullscreenLoading = this.$emit('open-load', {message: `已上传0%`, show: true})
                ...
                switch (_type) {
                    ...
                    case 2:
                        if (!(event.target.files[0].type.indexOf('video') < 0)) {
                            ...
                            if(this.isType == 'video'&&this.fullscreenLoading) { 
            ...
            handleUploadFile(form_data, url, e) {
                ...
                axios({
                    ...
                    onUploadProgress: function (progressEvent) {
                        ...
                        if(vm.isType == 'video') vm.fullscreenLoading = vm.$emit('open-load', {message: `已上传${progress}%`, show: true})
                    },
                }).then(res => {
                    e.target.value = ''
                    const RES_URL = this.isVideo ? videoUrl : url
                    this.resourse_url = RES_URL + '/' + this.video_url
                    if (this.type == 'video' && this.uploadConfig.time) {
                        let setTime = setInterval(() => {
                            if(this.$refs.vedioPlayer && this.$refs.vedioPlayer.duration) {
                                console.log(this.$refs.vedioPlayer.duration,'this.$refs.vedioPlayer.duration');
                ...
                get_sign(file_item.type, date, this.uploadConfig.bucket, this.uploadConfig.dir, file_item.name, 'POST')
                    .then(res => {
                        if (res.data.res_code == 1) {
                            ...
                            this.handleUploadFile(formData, encodeURI(this.isVideo ? videoOssHost : ossHost), event);
                        }
                    })
            },
        },
