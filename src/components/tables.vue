...
<!--                自定义展示按钮位置-->
                ...
                <span v-else
                      ...
                      :key="i">
                    <span v-if="handleBtnShow(column,row,t)" :class="handleBtnShowClass(column,row,t)">
                        <Button type="text" size="small" style="margin-right: 5px" @click.stop="show(row,index,t[1])">
                            {{handleBtnText(t,row,column)}}
                        </Button>
                    </span>
                </span>
                <Switch :class="column.isShow ? '' : 'operation_btn_show'" 
                 ...
                 <Switch :class="column.isShowAuth ? 'operation_btn_show' : ''"
                        v-if="column.isShowAuth" :true-value="1" :false-value="0" @on-change="changeAuth(row)"
                        v-model="row[column.switchKeyAuth]" size="large" >
                    <span slot="open">{{column.switchListAuth[0]}}</span>
                    <span slot="close">{{column.switchListAuth[1]}}</span>
                </Switch>
    ...
    export default {
        ...
        methods: {
            ...
            handleOperation(c, row) {
                ...
                if (c.hasOwnProperty('operationStudentNum')) return row.student_num ? list : [list[0], list[1], list[3], list[4], list[5]]
            },
            ...
            handleBtnShowClass(c, r, t) {
                if (!c.hasOwnProperty('operation_btn_hide')) {
                    ...
                    } else if (t&&t.length>0&&c.operation[0][0] == t[0]) return 'operation_btn_see'
            ...
            show(row, rowIndex, params, bool) {
                if(params === 'downTable') {
                    postData('code/getCodeHistory', {code_id: row.id, page_size: 20, page_num: 1, is_all: 1}).then(res => {
            ...
            handleColumns(c, type) {
                if (c && c.length > 0) {
                    if (type) {
                        if (this.isSerial && this.isSerial.isSerial && c[0].key != 'serial_number') c.unshift({title: '序号', key: 'serial_number', width: 70})
            ...
            changeAuth(row){
                this.$emit('table-swtich-auth', row)
            },
            handleBtnText(t, r, c) {
                // operation_state -- 处理 兑换码
                if(t[0]==='realNameAuthBtn') return r.realNameAuthState ? '实名认证' : ''
                else if (t&&Array.isArray(t[0])) {
            ...
            selectAllCancel(selection, row) {
                ...
                this.$emit('all-select-cancel', selection, row)
            },
